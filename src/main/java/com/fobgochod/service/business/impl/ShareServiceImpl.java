...
public class ShareServiceImpl implements ShareService {
    ...
    public String shareFileForever(FileInfo fileInfo) {
        ...
        shareRecord.setUrl(getShareUrl(shareRecord.getId()) + "/toAnyOne");
    ...
    public String shareFileTimeLimit(FileInfo fileInfo, int d) {
        ...
        shareRecord.setFileName(fileInfo.getName());
        shareRecord.setUrl(getShareUrl(shareRecord.getId().toString()) + "/toAnyOne");
    ...
    private String getShareUrl(String shareFileId) {
        String userName = UserUtil.getUserName();

        String url;
        if (StringUtils.hasText(envProperties.getBaseUri())) {
            url = String.format("%s/api/dmc/v1/buckets/shareFiles/files/%s", envProperties.getBaseUri(), shareFileId);
        } else {
            url = String.format("%s://%s:%s/api/dmc/v1/buckets/shareFiles/files/%s", request.getScheme(),
                    request.getServerName(), request.getServerPort(), shareFileId);
        }
        return url;
    }
}
